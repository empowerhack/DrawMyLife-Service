...
Draw My Life is currently hosted on [Heroku](https://www.heroku.com/), with images automagically hosted on [Cloudinary](http://www.cloudinary.com/). The `.env.production.example` file holds a template of the ENV vars required to be set on the live production environment.
...
The staging environment is set up as a replica of the Production environment, hosted on Heroku using Cloudinary for image storage.
